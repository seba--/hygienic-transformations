...
abstract class Module extends Nominal {
  ...
  def rename(renaming: Renaming) = rename(defs.map(d => ((renaming(d._1._1), d._1._2), d._2.rename(renaming))).toMap)
  def rename(renamingResult : Map[(Name, Boolean), Exp]) : Nominal

  def allNames = defs.values.foldLeft(Set[Name.ID]())(_ ++ _.allNames) ++ defs.keys.map(_._1.id)
...
case class InternalPrecedenceModule(id: String, imports: Set[Module], defs: Map[(Name, Boolean), Exp]) extends Module {
  ...
  def rename(renamingResult : Map[(Name, Boolean), Exp]) = InternalPrecedenceModule(id, imports, renamingResult)

  def exportedNames = imports.foldLeft(Set[Name.ID]())(_ ++ _.exportedNames) ++ defs.keys.filter(_._2).map(_._1.id).toSet
}
...
case class ExternalPrecedenceModule(id: String, imports: Set[Module], defs: Map[(Name, Boolean), Exp]) extends Module {
  ...
  def exportedNames = defs.keys.filter(_._2).map(_._1.id).toSet ++ imports.foldLeft(Set[Name.ID]())(_ ++ _.exportedNames)
}