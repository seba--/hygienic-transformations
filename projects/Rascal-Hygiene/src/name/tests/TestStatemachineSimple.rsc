module name::tests::TestStatemachineSimple
...
void visNamesCompiled1() = renderNames(resolveNames(p), p)
  when Prog p := compiled1();
  
void visNamesCompiled2() = renderNames(resolveNames(p), p)
  when Prog p := compiled2();
...
void visNamesFixed1() = renderNames(resolveNames(p), p)
  when Prog p := fixed1();
  
void visNamesFixed2() = renderNames(resolveNames(p), p)
  when Prog p := fixed2();
